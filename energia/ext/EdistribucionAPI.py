...
class Edistribucion():
    __session = None
    TEMP = 'tmp/'
    SESSION_FILE = TEMP + 'edistribucion.session'
    ACCESS_FILE = TEMP + 'edistribucion.access'
    __token = 'undefined'
    __credentials = {}
    ...
    def __init__(self, user, password, debug_level=logging.INFO):
        ...
        self.ACCESS_FILE = self.ACCESS_FILE + '-' + user
        self.SESSION_FILE = self.SESSION_FILE  + '-' + user
        ...
        try:
            with open(self.ACCESS_FILE, 'rb') as f:
                d = json.load(f)
    ...
    def login(self, forceLogin=True):
        logging.info('Loging')
        if (not self.__check_tokens() or forceLogin):
            self.__session = requests.Session()
    ...
    def get_meas(self, cont, cycle):
        data = {
            'message': '{"actions":[{"id":"95;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPoints","callingDescriptor":"markup://c:WP_Measure_Detail_v4","params":{"cupsId":"'+cont['Id']+'","dateRange":"'+cycle['label']+'","cfactura":"'+cycle['value']+'"},"longRunning":true}]}',
            }
        r = self.__command('other.WP_Measure_v3_CTRL.getChartPoints=1', post=data)
        return r['data']['lstData']

    #Cuando se entra en Consultar curva y te muestra el último día (anterior)
    #https://zonaprivada.edistribucion.com/areaprivada/s/sfsites/aura?r=9&other.WP_Measure_v3_CTRL.getInfo=1
    def get_consumo(self, cont):
        data = {
            'message': '{"actions":[{"id":"417;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getInfo","callingDescriptor":"markup://c:WP_Measure_Detail_v4","params":{"contId":"'+cont['Id']+'"},"longRunning":true}]}'
            }
        r = self.__command('other.WP_Measure_v3_CTRL.getInfo=1', post=data)
        return r['data']['lstData']

    #Consulta de un día especificado de la forma YYYY-MM-DD
    #https://zonaprivada.edistribucion.com/areaprivada/s/sfsites/aura?r=13&other.WP_Measure_v3_CTRL.getChartPointsByRange=1
    def get_consumo_byDate(self, cont, date):
        data = {
            'message': '{"actions":[{"id":"467;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPointsByRange","callingDescriptor":"markup://c:WP_Measure_Detail_Filter_By_Dates_v3","params":{"contId":"'+cont['Id']+'","type":"1","startDate":"'+date+'"},"version":null,"longRunning":true}]}'
            }
        r = self.__command('other.WP_Measure_v3_CTRL.getChartPointsByRange=1', post=data)
        return r['data']['lstData']

    #Consulta de la última semana
    #https://zonaprivada.edistribucion.com/areaprivada/s/sfsites/aura?r=16&other.WP_Measure_v3_CTRL.getChartPointsByRange=1
    def get_consumo_lastWeek(self, cont, date=datetime.strftime(datetime.now() - timedelta(1), '%Y-%m-%d')):
        data = {
            'message': '{"actions":[{"id":"522;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPointsByRange","callingDescriptor":"markup://c:WP_Measure_Detail_Filter_By_Dates_v3","params":{"contId":"'+cont['Id']+'","type":"2","startDate":"'+date+'"},"version":null,"longRunning":true}]}'
            
            }
        r = self.__command('other.WP_Measure_v3_CTRL.getChartPointsByRange=1', post=data)
        return r['data']['lstData']
        
    #Consulta del último mes
    #https://zonaprivada.edistribucion.com/areaprivada/s/sfsites/aura?r=19&other.WP_Measure_v3_CTRL.getChartPointsByRange=1
    def get_consumo_lastMonth(self, cont, date=datetime.strftime(datetime.now() - timedelta(1), '%Y-%m-%d')):
        data = {
            'message': '{"actions":[{"id":"569;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPointsByRange","callingDescriptor":"markup://c:WP_Measure_Detail_Filter_By_Dates_v3","params":{"contId":"'+cont['Id']+'","type":"3","startDate":"'+date+'"},"version":null,"longRunning":true}]}'
            
            }
        r = self.__command('other.WP_Measure_v3_CTRL.getChartPointsByRange=1', post=data)
        return r['data']['lstData']
        
    #Consulta por rango de fechas, especificados de la forma YYYY-MM-DD
    #https://zonaprivada.edistribucion.com/areaprivada/s/sfsites/aura?r=19&other.WP_Measure_v3_CTRL.getChartPointsByRange=1
    def get_consumo_byRange(self, cont, startDate, endDate):
        data = {
            'message': '{"actions":[{"id":"641;a","descriptor":"apex://WP_Measure_v3_CTRL/ACTION$getChartPointsByRange","callingDescriptor":"markup://c:WP_Measure_Detail_Filter_Advanced_v3","params":{"contId":"'+cont['Id']+'","type":"4","startDate":"'+startDate+'","endDate":"'+endDate+'"},"version":null,"longRunning":true}]}'
